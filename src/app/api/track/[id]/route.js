...
export async function GET(_req, { params }) {
  try {
    ...
    const track = await Track.findOne({ _id: params.id, user: user._id });
...
export async function PUT(req, { params }) {
  try {
    ...
    return NextResponse.json({ message: "Track diupdate", data: updated });
  } catch (err) {
    console.error(err);
    return NextResponse.json({ message: "Error server" }, { status: 500 });
  }
...
export async function DELETE(_req, { params }) {
  try {
    ...
    return NextResponse.json({ message: "Track dihapus" });
  } catch (err) {
    console.error(err);
    return NextResponse.json({ message: "Error server" }, { status: 500 });
  }
